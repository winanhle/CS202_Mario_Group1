...
void GameWorld::update(float deltaTime)
{
    ...
    // 2.5. Chạm FLAGPOLE → sang stage kế tiếp (map mới tải lại từ đầu)
    checkFlagpoleCollision();

    // 3. FireBar obstacles rotate and damage player
    if (m_fireBarManager)
        m_fireBarManager->update(deltaTime);

    // 4. Enemy check va chạm với player position MỚI
    if (m_enemyManager)
        m_enemyManager->update(deltaTime);

    // 5. Item check va chạm với player position MỚI
    if (m_itemManager)
        m_itemManager->update(deltaTime);


    // 6. Kiểm tra & xử lý death / respawn / game over
    checkAndHandleDeath();
